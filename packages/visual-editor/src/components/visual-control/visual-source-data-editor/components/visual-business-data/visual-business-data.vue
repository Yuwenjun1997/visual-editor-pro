      ...
      <el-form size="small" label-position="top">
        <el-form-item label="数据类型">
          <el-radio-group v-model="businessType">
            <el-radio-button label="商品" value="products" />
            <el-radio-button label="文章" value="articles" />
          </el-radio-group>
        </el-form-item>
        ...
        <el-form-item>
          <el-button type="primary" :loading="loading" @click="handleLoad"> 载入数据 </el-button>
          <span style="color: #909399" class="ve-ml-2 ve-text-xs"> 载入当前数据管理中的记录,确认后生效于画布 </span>
        </el-form-item>
      </el-form>
      <div v-if="modelValue.businessDataRef" class="ve-mt-2">
        <div style="color: #67c23a; font-size: 12px">
          已绑定:{{ modelValue.businessDataRef.businessType === 'products' ? '商品' : '文章' }} &nbsp;·&nbsp;{{
            modelValue.businessDataRef.refType === 'all' ? '全部分类' : '分类数据'
          }}
        </div>
        <div style="color: #909399; font-size: 12px; margin-top: 4px">页面保存时会用最新数据重新生成快照</div>
      </div>
    </template>
    <el-empty v-else :image-size="60" description="未配置数据服务" />
  </div>
</template>
...
const categoryType = computed<'product' | 'article'>(() => (businessType.value === 'products' ? 'product' : 'article'))
...
const handleLoad = async () => {
  ...
  try {
    ...
    modelValue.value.customJsonData =
      modelValue.value.customDataType === 'VisualObject' ? JSON.stringify(rows[0] ?? {}) : JSON.stringify(rows)
