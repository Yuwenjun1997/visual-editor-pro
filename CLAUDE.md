...
`visual-editor-pro` is a **pnpm + Turborepo monorepo** for a Vue 3 + TypeScript drag-and-drop **visual page editor** (low-code). Users drag "visual blocks" from a left palette onto a stage, configure each block's props/styles in a right panel, and the editor serializes the page into a JSON schema. Built with Vite, Pinia, Vue Router, Tailwind CSS v3 (per-package prefix isolation), Element Plus (web + editor), and shadcn-vue (visual-ui).
...
packages/visual-ui/     @visual/ui   组件库（可独立发布，shadcn-vue，零 Element Plus）
```
...
自包含 Vue 插件：`src/index.ts` 导出 `install(app, options)` 并全局注册 `VisualApp` / `VisualText` / `VisualFlex` / … 等 21 个渲染组件；自有 hooks（`useTheme`、`useSafeArea`、`useVisualRequest`）、utils（`cn`、`toast`）、`v-*` 前缀 SCSS 工具库、fonts（bootstrap-icons）、`components/ui/`（shadcn-vue 基元）。
...
编辑器外壳与逻辑整体作为一个可复用包，导出 `EditorLayout`（三栏外壳）、`EditorStage`（编辑器舞台）、`PreviewScenario`（预览页）、`DevScenario`（开发沙盒）、`setupVisual`（安装 visual-ui + 主题挂载）、`registryComponent`、`useViusalStore`（Pinia store）与各 `useXxx` hooks、`*` 类型。

内部结构沿用原布局：`layout/`（left/right/top 窗口）、`store/`（`useViusalStore`）、`hooks/`、`utils/`（`visual.control.ts` 控件工厂、`visual.registry.ts` 全局配置）、`types/visual-editor.ts`、`configs/`、`packages/modules/`（每块的编辑器元数据）、`schemas/`、`components/`（编辑控件，深依赖 Element Plus）、`views/{index,preview,dev}`（三个场景）。

- 依赖 Element Plus（`dependencies`，与宿主同版本经 pnpm 去重为单实例）；peer：`vue` / `pinia` / `vue-router`（宿主提供运行时实例）。
- `src/hooks/useMountThemeToRoot.ts`：把 visual-ui 的 `--v-*` 主题变量挂到 `document.documentElement`，供 `.visual-app` 之外的编辑面板（icon-picker / theme-picker）与 teleport 到 body 的弹层取用；由 `setupVisual` 调用。

### `@visual/web` — web 应用薄壳（`apps/web/`）

导装配为主：`main.ts`（装配顺序见下）、`router/index.ts`（路由导三个场景组件）、`plugins/{element-ui,monaco-editor,iconify}`、宿主 SCSS/Tailwind 入口、`public/`（宿主资产，见下）。路由结构：`/`（EditorLayout → EditorStage）、`/dev`（DevScenario）、`/preview`（PreviewScenario）。

`main.ts` 的 CSS 引入顺序是**级联契约**：EP 样式 → monaco → 两库 SCSS side-effect（`@visual/ui`、`@visual/editor`）→ 两库 Tailwind 产物（`style.css`）→ 应用自带 tailwind/preflight → iconfont。
...
- 编辑器的 `.vue` 模板中，Tailwind 布局类必须写 `ve-` 前缀（如 `ve-flex-1`、`ve-h-screen`）；应用模板用 `wa-`。visual-ui 的 shadcn 工具类用 `vu-`。
- shadcn HSL 变量（`--background` 等）与 `--v-*` 主题变量：由 `visual-app.vue` 在 `.visual-app` 子树运行时注入；编辑器侧再经 `useMountThemeToRoot` 挂到 `<html>`。
...
2. **仅用 shadcn-vue**：新增 UI 基元仅基于 shadcn-vue（reka-ui / vue-sonner），落于 `packages/visual-ui/src/components/ui/`（`components.json` 已在包内）。
3. **样式**：Tailwind 工具类统一 `vu-` 前缀；其余自定义样式仅用包内 SCSS（`v-*`）。不新增生成全局 reset 的样式。
4. **原因**：组件库会脱离当前项目单独使用；除 shadcn-vue 依赖体系外不引入其他 UI 库或全局样式。
